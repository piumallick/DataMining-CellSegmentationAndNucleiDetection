...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    negative_indices = tf.compat.v1.random_shuffle(negative_indices)[:negative_count]
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    refined_rois = tf.compat.v1.to_int32(tf.compat.v1.rint(refined_rois))
    ...
    def nms_keep_map(class_id):
        ...
        class_keep = tf.image.non_max_suppression(
                tf.compat.v1.to_float(tf.gather(pre_nms_rois, ixs)),
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = np.round(np.array(Image.fromarray(class_mask.astype(float)).resize((gt_w, gt_h), resample=Image.NEAREST)) / 255.0).astype(bool)
            # np.round(scipy.misc.imresize(class_mask.astype(float), (gt_h, gt_w), interp='nearest') / 255.0).astype(bool)
        ...
        mask = np.array(Image.fromarray(m.astype(float)).resize((config.MASK_SHAPE[1], config.MASK_SHAPE[0]), resample=Image.NEAREST)) / 255.0
...
class MaskRCNN():
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        """
        import h5py
        from keras.engine import topology, saving
