...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        m = np.array(Image.fromarray(m.astype(float)).resize((mini_shape[1],mini_shape[0]), resample=Image.BILINEAR))
...
def unmold_mask(mask, bbox, image_shape):
    ...
    #mask = skimage.transform.resize(mask, (y2 - y1, x2 - x1)).astype(np.float32) / 255.0
    mask = np.array(Image.fromarray(mask).resize((x2 - x1, y2 - y1), resample=Image.BILINEAR)).astype(np.float32) / 255.0
